...
with tab2:
    ...
    fig3 = px.scatter(df_rfm, x="Frequency", y="Monetary", color="Segment", height=400)
...
with tab3:
    ...
    fig1 = px.pie(behavior_counts, names="Type", values="Count", height=350)
    col1.plotly_chart(fig1, use_container_width=True)

    fig2 = px.histogram(df_behavior, x="days_since_last_purchase", nbins=40, height=350)
    col2.plotly_chart(fig2, use_container_width=True)
...
with tab4:
    ...
    high_risk = df_churn[df_churn["churn"] == "High"]

    col2.metric("High Risk Customers", len(high_risk))
    col2.metric("Revenue at Risk", f"₹{high_risk['monetary'].sum():,.0f}")

    seg_churn = df_churn.groupby(["segment", "churn"]).size().reset_index(name="count")

    fig2 = px.bar(seg_churn, x="segment", y="count", color="churn", height=400)
...
with tab5:
    ...
    fig1 = px.line(monthly, x="purchase_at", y="total_revenue", height=400)
...
with tab6:
    ...
    if customer_id:
        ...
        if rfm_data.empty:
            ...
            col2.metric("Churn", churn_data["churn"].values[0] if not churn_data.empty else "NA")
            col3.metric("Revenue", f"₹{orders_data['total_revenue'].sum():,.0f}")

            if not orders_data.empty:
                fig = px.line(orders_data.sort_values("purchase_at"),
                              x="purchase_at", y="total_revenue", height=350)
